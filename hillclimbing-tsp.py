...
list = [0,7,4,2,5,5,2,6,3,4,6,3,3,7,3,9,10,8,4,6,2,10,9,9,3,6,1,5,4,2,2,5,1,6,5,7,7,7,1,1,4,6,7,5,9,8,10,6,4,2,
...
# Convert list into 2D array matrix
travelling_salesman = np.array(list).reshape(-1, 50).tolist()
####### Step 1 : Initial Solution: creates a random route from 0 to 50 with unique items.

# Pick a random route between 1 and 50 unique ; flatten to 1 level and convert back into an array
random_route = np.random.choice(np.arange(1, 51), replace=False, size=(1, 50))[0].tolist()
...
def newRoute(route):
    ...
    route[iteration_index+1], route[iteration_index+2] = route[iteration_index+2], route[iteration_index+1]
    iteration_index += 1
    return route 
...
def calculateCost(route):
    ...
    for index, city in enumerate(route):
        ...
        if next_city_index < len(route):
            next_city = route[next_city_index]
            # print(travelling_salesman[city-1][next_city-1])
            route_cost += travelling_salesman[city-1][next_city-1]
    # add the cost of last city -> first city     
    route_cost += travelling_salesman[len(route)-1][starting_city-1]
    return route_cost
...
# Apply random start hill climb 
while number > 0:
    print("Solution number ", 11-number)
    best_route = hillClimb(best_route)
    print("Total Cost: ", calculateCost(best_route), "Route: ", best_route, "\n")
    number -= 1
